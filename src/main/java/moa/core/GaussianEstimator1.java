 ...
 * @version $Revision: 7 $
 */
public class GaussianEstimator1 extends AbstractMOAObject {
    ...
    public void addObservation(double value, double weight, double sumofinst, double mea, double vars) {
        if (Double.isInfinite(value) || Double.isNaN(value)) {
            return;
        }
        if (sumofinst-1.0 > 0.0) {
          
            this.mean = mea + weight * (value - mea) / sumofinst; 
            this.varianceSum = vars +  weight * (value - mea) * (value - this.mean);
    ...
    public void addObservations(GaussianEstimator1 obs) {
    ...
    public double getStdDev(double suminst) {
        return Math.sqrt(getVariance(suminst));
    }

    public double getVariance(double suminst) {
        return this.weightSum > 1.0 ? this.varianceSum / (this.weightSum - 1.0)
                : 0.0;
    }

    public double probabilityDensity(double value, double sumInst) {
        this.weightSum = sumInst;
        if (this.weightSum > 0.0) {
            double stdDev = getStdDev(sumInst);
            if (stdDev > 0.0) {
