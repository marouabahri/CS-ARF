 ...
 * @version $Revision: 1 $
 */
public class AdaptiveRandomForestRE extends AbstractClassifier implements MultiClassClassifier {
    ...
    // TODO: Add an option to disable the resampling
...
//        BasicClassificationPerformanceEvaluator classificationEvaluator = (BasicClassificationPerformanceEvaluator) getPreparedClassOption(this.evaluatorOption);
    ...
    protected final class ARFBaseLearner {
        ...
        private void populateCounterArray(Instance instnc) {
            for (int i = 0; i < instnc.numClasses(); i++) {
                this.counterArray.add(i, 0);
            }
        }

        private void updateCounterArray(Instance instnc) {
            this.counterArray.set((int) instnc.classValue(), (this.counterArray.get((int) instnc.classValue()) + 1));
...
//                double weightAux = 100 - (this.counterArray.get((int) instance.classValue()) * 100 / instancesSeen);
//                weight = weightAux / (100 * weight);
                weight = (double) this.counterArray.get((int) instance.classValue()) / instancesSeen * weight;
//                weight =  -1 * ((double) this.counterArray.get((int) instance.classValue()) - instancesSeen ) / instancesSeen * weight;
